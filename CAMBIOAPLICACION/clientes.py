...
class Clientes(QWidget):
    ...
    def eliminar_cliente(self, codigo):
        confirm = QMessageBox.question(self, "Confirmar eliminación", "¿Está seguro que desea eliminar este cliente?", QMessageBox.Yes | QMessageBox.No)
        if confirm == QMessageBox.Yes:
            try:
                ...
                connection.close()
                self.cargar_clientes()
                QMessageBox.information(self, "Éxito", "El cliente se ha eliminado correctamente.")
            except cx_Oracle.DatabaseError as e:
                error, = e.args
    ...
    def insertar_cliente_en_db(self, cliente):
        try:
            ...
            cursor.close()
            connection.close()
            QMessageBox.information(self, "Éxito", "El nuevo cliente se ha añadido correctamente.")
        except cx_Oracle.DatabaseError as e:
            error, = e.args
    ...
    def actualizar_cliente_en_db(self, cliente):
        try:
            ...
            cursor.close()
            connection.close()
            QMessageBox.information(self, "Éxito", "El cliente se ha actualizado correctamente.")
        except cx_Oracle.DatabaseError as e:
            error, = e.args
