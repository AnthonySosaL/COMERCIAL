...
class Productos(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_crear_producto.setStyleSheet("background-color: #f8d7da; color: black; font: bold 12pt 'Arial';")
    ...
    def editar_producto(self, row_data):
        ...
        add_form_item("Saldo Inicial", str(row_data[3]) if row_data else "")
        add_form_item("Ingresos", str(row_data[4]) if row_data else "")
        add_form_item("Egresos", str(row_data[5]) if row_data else "")
        add_form_item("Ajustes", str(row_data[6]) if row_data else "")
        add_form_item("Saldo Final", str(row_data[7]) if row_data else "")
        add_form_item("Costo U.M.", str(row_data[8]) if row_data else "")
        add_form_item("Precio U.M.", str(row_data[9]) if row_data else "")
